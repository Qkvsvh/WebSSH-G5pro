<template>
  <el-container>
          ...
          <el-header 
            ...
            <div class="nav">
              ...
              <div class="right" style="text-align: right">
                <el-button-group>
                  <el-button type="primary" :icon="Refresh" @click="onUpdateClick" title="系统更新（GitHub 自动更新）"></el-button>
          ...
          <!-- 折叠态：屏幕右边缘感应条（桌面鼠标移到最右缘即展开；手机用右边缘中间向左滑动唤出） -->
          <div
            v-show="data.navCollapsed"
            class="nav-edge-trigger"
            @mouseenter="data.navCollapsed = false"
            title="导航栏"
          ></div>

          <!-- 首次进入：操作提示引导动画（localStorage 标记，只出现一次） -->
          <transition name="nav-guide-fade">
            <div
              v-if="data.showNavGuide"
              class="nav-guide-mask"
              @click.self="dismissNavGuide"
            >
              <div class="nav-guide-card">
                <div class="nav-guide-gesture">
                  <span class="nav-guide-hand"></span>
                </div>
                <p class="nav-guide-text">从屏幕<strong>右侧边缘</strong>向左滑动<br />即可唤出导航栏</p>
                <button class="nav-guide-ok" @click="dismissNavGuide">知道了</button>
              </div>
            </div>
          </transition>
                ...
                <div class="progress-info-table">
                  ...
                  <div class="info-table-row">
                    ...
                    <span class="info-table-value speed">
                      {{ updateProgress.status === 'downloading' ? formatSpeed(updateProgress.speed ?? 0) : '--' }}
                    </span>
                  </div>
...
let data = reactive({
  navCollapsed: true,
  navPeek: true, // 默认只露一小条边（peek），向上滑才完整显示，避免遮挡顶部内容
  showNavGuide: false, // 首次进入引导提示（localStorage 标记控制只出现一次）
  mode: Mode.create,
  id: 0,
  name: "",
  address: "",
...
async function testProxySpeed() {
  ...
  try {
    const testURL = updateVersionInfo.value.release_url || "https://github.com/Qkvsvh/WebSSH-G5pro/releases/latest";
...
// 系统更新（原版 GitHub 自动更新）：点击顶部「系统更新」按钮触发。
// 先查询 GitHub 最新版本，有更新则确认后下载替换并重启；已是最新则提示。
async function onUpdateClick() {
  ...
  if (!info) {
    ElMessageBox.confirm(
      "无法获取版本信息，是否仍尝试从 GitHub（Qkvsvh/WebSSH-G5pro）检查并下载更新？",
      "系统更新",
...
/**
 * 根据页面滚动方向控制折叠展开按钮的显隐：
 * 向下滑 -> 仅露出一点（navPeek=true）；向上滑 -> 完整显示（navPeek=false）
 */
let lastScrollTop = 0;
function onPageScroll() {
  ...
  data.navPeek = delta > 0; // 向下 delta>0 仅露一点；向上 delta<0 全显
  lastScrollTop = st;
}

/**
 * 屏幕右边缘中间滑动唤出导航栏（折叠态）
 * - 手机/平板：从右边缘（屏幕高度中部区域）向左滑 -> 展开
 * - 桌面鼠标无触摸时由 .nav-edge-trigger（右缘）hover 负责
 */
const EDGE_THRESHOLD = 24; // 距屏幕右边缘多少 px 内算"边缘"
const SWIPE_MIN = 50;      // 最小横向滑动距离（px）
const EDGE_V_BAND = 0.2;   // 起点 Y 须在屏幕高度 20%~80% 的中部，避开 iPad 上下系统手势区
...
function onEdgeTouchEnd(e: TouchEvent) {
  ...
  // 起点在右边缘 + 起点 Y 在中部 + 向左滑 -> 展开
  const fromRight =
    edgeSwipeStartX >= window.innerWidth - EDGE_THRESHOLD &&
    dx <= -SWIPE_MIN &&
    edgeSwipeStartY >= window.innerHeight * EDGE_V_BAND &&
    edgeSwipeStartY <= window.innerHeight * (1 - EDGE_V_BAND);
  if (fromRight) {
...
/**
 * 关闭首次引导提示并写入本地标记（只出现一次）
 */
function dismissNavGuide() {
  data.showNavGuide = false;
  try {
    localStorage.setItem("webssh_nav_guide_v1", "1");
  } catch (e) {
...
onMounted(() => {
  ...
  // 首次进入引导：未看过则弹出操作提示动画（localStorage 标记控制只出现一次）
  try {
    if (!localStorage.getItem("webssh_nav_guide_v1")) {
      data.showNavGuide = true;
    }
  } catch (e) {
...
/* 折叠态：屏幕右边缘感应条（桌面上把鼠标移到最右缘即展开；手机用右边缘中间向左滑动唤出） */
.nav-edge-trigger {
  position: fixed;
  top: 0;
  right: 0;
...
.nav-guide-card {
  ...
  background: linear-gradient(135deg,
    hsl(calc(var(--theme-primary-h, 201) * 1), var(--theme-primary-s, 100%), 30%),
    hsl(calc(var(--theme-primary-h, 201) * 1), calc(var(--theme-primary-s, 100%) * 0.8), 44%));
  color: #f1f5f9;
  box-shadow: 0 24px 60px rgba(2, 6, 23, 0.5);
  animation: navGuidePop 0.32s ease-out both;
}
@keyframes navGuidePop {
...
.nav-guide-hand {
  ...
  background: radial-gradient(circle at 32% 30%, #bae6fd, #0ea5e9);
  box-shadow: 0 6px 16px rgba(2, 6, 23, 0.4);
  animation: navSwipe 1.8s ease-in-out infinite;
}
.nav-guide-hand::after {
  content: "";
  position: absolute;
  left: -11px;
  top: 50%;
...
@keyframes navSwipe {
  ...
  55% { transform: translate(-120px, -50%); }
...
@media (prefers-color-scheme: dark) {
  .nav-guide-card {
    background: linear-gradient(135deg,
      hsl(calc(var(--theme-primary-h, 201) * 1), var(--theme-primary-s, 100%), 22%),
      hsl(calc(var(--theme-primary-h, 201) * 1), calc(var(--theme-primary-s, 100%) * 0.8), 34%));
  }
}
